...
const NumberingList = styled.div`
  position:absolute;
  right:55px;
  top:5px;
  color:red;
      ...
      >
        ...
        <Portlet>
          <Row>
            <Col>
              <CustomSelectBarang 
                readonly={(dataBarang.length ? true : false)} 
                setSelected={getBarangSelected} 
                initialValue={initSelected} 
                itemList={props.barangList} 
                label={"Barang"}/>
                  ...
                  {dataBarang.map((barang, index)=>{
                    return (
                        <ListGroup.Item className="mb-1"  key={index} style={(barang.flag == 0 ? styleRemove : {position:"relative"})}>
                          ...
                          {
                            (index + 1)%5 == 0 && index > 0?
                            <NumberingList>{index+1}</NumberingList>
                            : <></>
                          }
